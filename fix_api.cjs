...
if (!content.includes('async getAssetPlatforms()')) {
  content = content.replace(/export const apiService = \{/, "export const apiService = {\n" + apiStr);
  
  // also fix SyncAction type
  content = content.replace(/type SyncAction =/, "type SyncAction = { type: 'ADD_ASSET' | 'UPDATE_ASSET' | 'DELETE_ASSET', payload: any } |");
  
  fs.writeFileSync('src/lib/supabase.ts', content);
  console.log("Patched API service object");
}
